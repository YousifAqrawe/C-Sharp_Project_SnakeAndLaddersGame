...
namespace GUI_Class
{
    public partial class SpaceRaceForm : Form
    {
        ...
        private static void MapSquareNumToScreenRowAndColumn(int squareNum, out int screenRow, out int screenCol)
        {
            ...
            const int RowsOfSquares = 7; // there are 7 Rows in the board 
            const int ColumnsOfSquares = 8; // there are 8 columns in the board

            screenRow = RowsOfSquares - 1 - Math.DivRem(squareNum, ColumnsOfSquares, out screenCol);

            if ((screenRow & 1) != 0)
            {
                screenCol = ColumnsOfSquares - 1 - screenCol;
            }
        ...
        private void DetermineNumberOfPlayers()
        {
            ...
            //SpaceRaceGame.NumberOfPlayers = comboBox1.SelectedItem + 2;
            string slectedNumber = (string)comboBox1.SelectedItem;

            // Parse string to a number
            int NumberOfPlayer = int.Parse(slectedNumber);
            SpaceRaceGame.NumberOfPlayers = NumberOfPlayer;
        ...
        {
            //  Square squareNumber = SpaceRaceGame.Players[playerNumber].Location;
        ...
        private void UpdatePlayersGuiLocations(TypeOfGuiUpdate typeOfGuiUpdate)
        {
            ...
            //   
            int SpecificSquareNumber = 0;
            for (int i = 0; i < SpaceRaceGame.Players.Count; i++)

            {
                SpecificSquareNumber = GetSquareNumberOfPlayer(i);
                SquareControl squareControl = SquareControlAt(SpecificSquareNumber);

                if (TypeOfGuiUpdate.AddPlayer == typeOfGuiUpdate)
                {
                ...
                //squareNmuber = GetSquareNumberOfPlayer(0);
            }
        ...
        {
            RollDiceButton.Click += new EventHandler (button2_Click) ;
            UpdatePlayersGuiLocations(TypeOfGuiUpdate.RemovePlayer);
            SpaceRaceGame.PlayOneRound();
            UpdatePlayersGuiLocations(TypeOfGuiUpdate.AddPlayer);
            UpdatesPlayersDataGridView();
            //if (SpaceRaceGame.NumberOfPlayers == 2)
            //{
                
            //}
            //if (SpaceRaceGame.NumberOfPlayers == 3)
            //{

            //}
            //for (int i = 0; i < Board.FINISH_SQUARE_NUMBER ; i++)
            //{
